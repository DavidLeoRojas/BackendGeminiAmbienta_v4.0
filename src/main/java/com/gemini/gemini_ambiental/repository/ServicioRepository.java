...
public interface ServicioRepository extends JpaRepository<Servicio, String> { // String para UUID
    ...
    // --- ✅ MÉTODOS EXPLÍCITOS PARA LA TAREA PROGRAMADA ---
    @Query("SELECT s FROM Servicio s WHERE s.estado = :estado AND s.fecha >= :fechaInicio AND s.fecha <= :fechaFin ORDER BY s.fecha, s.hora")
    List<Servicio> findByEstadoAndFechaBetween(@Param("estado") Servicio.EstadoServicio estado, @Param("fechaInicio") LocalDate fechaInicio, @Param("fechaFin") LocalDate fechaFin);

    @Query("SELECT s FROM Servicio s WHERE s.estado = :estado AND s.fecha < :fechaLimite ORDER BY s.fecha, s.hora")
    List<Servicio> findByEstadoAndFechaBefore(@Param("estado") Servicio.EstadoServicio estado, @Param("fechaLimite") LocalDate fechaLimite);
    // --- FIN MÉTODOS TAREA PROGRAMADA ---

    // --- ✅ MÉTODO PARA ACTUALIZAR SOLO EL ESTADO ---
    @Modifying
    @Query("UPDATE Servicio s SET s.estado = :nuevoEstado WHERE s.idServicio = :idServicio")
    int actualizarEstado(@Param("idServicio") String idServicio, @Param("nuevoEstado") Servicio.EstadoServicio nuevoEstado);
    // --- FIN MÉTODO ESTADO ---
}